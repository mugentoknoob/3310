...
const myBot = ['u6bb132762c40732bb60472c2640e4bb0','ud3949568f78f169262cd9c55231ae5fd','u2cc9b3a617d9d863a3d218cec11b8afc'];
const banList = [];//Banned list
var groupList = new Array();//Group list
var vx = {};var midnornama,pesane,kickhim;var waitMsg = "no";//DO NOT CHANGE THIS
const imgArr = ['png','jpg','jpeg','gif','bmp','webp'];//DO NOT CHANGE THIS
var komenTL = "AutoLike by GG\nline://ti/p/~.guss"; //Comment for timeline
...
class LINE extends LineAPI {
    constructor() {
        ...
        this.keyhelp2 = "\n\
#====คำสั่ง====\n\n\
❁> !setpoint<ตั้งจุดนับคน\n\
❁> !check<เช็คคนอ่าน\n\
❁> !clear<ล้างรายชื่อคนอ่าน\n\
❁> !tagall<แท็กสมาชิกทุกคน\n\
❁> !botcontact\n\
❁> !botleft *ADMIN*\n\
❁> broadcast *ADMIN*\n\
❁> !curl\n\
...
\n\n>||=========================||";
    ...
    async textMessage(textMessages, seq, param, lockt) {
		...
		if(vx[1] == "!getvdopf" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"# CANCELLED");
		...
		if(vx[1] == "!youtube" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				...
				if(hasil == "Error: no_media_found"){
			    	infDl.text = "ล้มเหลว! อาจเป็น URL ผิด...";
				}else{
...
Link Download: "+idU.id+"\n";
		...
		if(txt == "!youtube" && !isBanned(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
			    this._sendMessage(seq,"วิธีใช้งาน | ตัวอย่าง !youtube ลิงค์ที่ต้องการดาวน์โหลด !");
				vx[2] = "arg1";
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
...
\n";
		...
		if(txt == "!yousound" && !isBanned(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
			    await this._sendMessage(seq,"วิธีใช้งาน | ตัวอย่าง !yousound ลิงค์ที่ต้องการดาวน์โหลด ! ?");
				this._sendMessage(seq,"Ok, ส่งลิงค์");
				vx[2] = "arg1";
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(vx[1] == "!botleft" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				...
				vx[3] = txt;
				this._sendMessage(seq,"OK, ชื่อกลุ่ม ?");
				vx[2] = "arg2";
		...
		if(vx[1] == "!grouputil" && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			switch(txt){
				...
				case 'ticket':
					...
					M.text = "เลือกหมายเลขกลุ่มด้านล่าง !";
				...
				}else{this._sendMessage(seq,"ไม่มีกลุ่ม !");}
			}
		}
		...
		if(txt == '0101' && lockt == 1) {//Jangan dicoba (gk ada efek)
            ...
            for (var i = 0; i < listMember.length; i++) {
                if(listMember[i].mid==param){
					let namanya = listMember[i].dn;
					seq.text = 'สวัสดีคุณ @'+namanya+'\nเข้าใหม่! \nแนะนำตัวให้เพิ้ลๆรู้จักกันหน่อย..😘\nมาจากสายการบินไหนคร้าบ';
					let midnya = listMember[i].mid;
